...
if (!id) {
    ...
    fetch(`http://localhost:3000/plantas/${id}`)
        .then(res => res.json())
        .then(planta => {
            const container = document.getElementById('detalhes-container');const params = new URLSearchParams(window.location.search);
const id = params.get('id');
...
if (!id) {
    ...
    `;
            ...
            // Corrigido: evento do botão Voltar sempre disponível
        ...
        `;
                ...
                // Voltar continua funcionando durante edição
                document.getElementById('btnVoltar').onclick = () => {
                    window.location.href = "./index.html";
                };
                ...
                document.getElementById('formAtualizar').onsubmit = function (e) {
                    ...
                    fetch(`http://localhost:3000/plantas/${id}`, {
                        ...
                        .then(async res => {
                            if (res.ok) {
                                exibirMensagem('Planta atualizada com sucesso!', "#4CAF7A");
                                setTimeout(() => location.reload(), 1200);
                            } else if (res.status === 401 || res.status === 403) {
                                exibirMensagem('Você não tem autorização para atualizar!', "#d32f2f");
                            } else {
                                const erro = await res.json();
                                exibirMensagem(erro.error || 'Erro ao atualizar planta!', "#d32f2f");
                            }
                        })
                        .catch(err => {
                            exibirMensagem('Erro ao conectar com o servidor!', "#d32f2f");
                            console.error(err);
                        });
                };
            };
